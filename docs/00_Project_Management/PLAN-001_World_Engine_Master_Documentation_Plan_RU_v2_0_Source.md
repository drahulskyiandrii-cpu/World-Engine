...
Версия 2.0 · Русский · 2026-09-05 · Текущий управляющий документ
...
Прежняя таблица указывала итог `28 / 92`, хотя перечисляла 114 позиций. После появления ADR-008 и ADR-009 фактический объём программы составляет 116 поставляемых артефактов. На дату этой версии подтверждены 33 из 116.
...
| Завершено | 28 | 33 | Активный канонический индекс и проверенные файлы |
...
| Стандарты проекта | 0 | 10 | Корневой README не засчитывается как полный стандарт |
...
| **Итого** | **33** | **116** | **28,4 % поставляемых артефактов** |
...
|---|---|---|---|
| F0 Master Control | PLAN-001 RU/DE, индекс и QA | Реальный реестр, порядок и критерии | Выполняется этой версией |
| F1 Governance | DOC-009; STD-001–008 | Общий язык, правила документов и Git | Planned |
...
После публикации PLAN-001 следует закрывать не все 83 оставшихся документа подряд, а минимальный foundation критического пути. Это даст достаточную определённость до кода и не превратит проект в бесконечное написание документов.

| Порядок | Пакет | Зачем сейчас | Выходной критерий |
|---:|---|---|---|
| 1 | F1 Governance | Устраняет терминологические и процессные расхождения | DOC-009 и STD-001–008 Current |
...
| ID | Артефакт | Статус | Класс | Условие закрытия |
|---|---|---|---|---|
| STD-001 | Glossary | Planned | F | Canonical terms RU/DE/EN and forbidden ambiguities |
| STD-002 | Naming Convention | Planned | F | Names for modules, events, APIs, schemas, files and tests |
...
После завершения QA этой версии следующим рабочим пакетом является F1 Governance. Его рекомендуемая внутренняя последовательность: STD-001 Glossary; STD-002 Naming Convention; STD-004 Documentation Standard; DOC-009 Developer Journal; STD-003 Coding Guidelines; STD-005 Git Workflow; STD-006 Commit Convention; STD-007 Branch Strategy. STD-008–010 можно закрыть ближе к первому release baseline.
...
Количество строк по этапам: `9 + 12 + 9 + 15 + 8 + 6 + 6 + 7 + 6 + 4 + 10 + 6 + 10 + 8 = 116`. Завершённые строки: `8 + 12 + 9 + 4 = 33`. Остаток: `116 − 33 = 83`. Доля выполнения: `33 / 116 = 28,4 %`.
