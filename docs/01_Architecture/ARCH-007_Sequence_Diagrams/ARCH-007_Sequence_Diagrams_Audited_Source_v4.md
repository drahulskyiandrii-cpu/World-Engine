# ARCH-007 — Sequence Diagrams — Audited Source v4

**Status:** Audit branch final-source candidate  
**Baseline:** ADR-002, ADR-005 + amendment, ADR-006, ADR-008; ARCH-004/005 audited model; ARCH-011 audited Save model  
**Date:** 2026-09-01
...
Static dependency direction remains defined by ARCH-005. Runtime arrows in this document must not be copied into the static dependency graph without contract-ownership analysis.
...
## SD-005 — Load World / Atomic Logical Activation
...
7. Rollback/recovery must restore an acceptable pre-load or controlled recovery state.
...
Permitted implementation strategies include shadow-state + atomic swap, transaction-like participant barrier, reversible apply with guaranteed rollback, immutable snapshot replacement, or another explicitly tested equivalent mechanism.
...
1. Game Adapter communicates with the application through ingress/output capability ports; it does not call domain module implementations directly.
...
9. Application Host / Composition Root owns top-level construction/lifecycle wiring and application activation/readiness gates.
10. Restore activation is logically all-or-nothing across all required SaveParticipants.
11. Runtime arrows in this document must not be copied into ARCH-005 as static dependencies without explicit contract analysis.
...
- load flow without explicit staging/cross-participant validation;
- partial live-state restore without atomic logical activation and rollback/recovery semantics.

## 4. Finalization requirement

The final RU+DE ARCH-007 Complete artifacts must be regenerated from this v4 source, and SD-005 terminology must remain semantically identical to ARCH-011 audited Save System Architecture.
